...
export async function getNotificationsByUser(
  user_id: string
): Promise<NotificationBE[]> {
  const res = await api.get<NotificationBE[]>(
...
export async function getUnreadNotifications(
  ...
  const res = await api.get<NotificationBE[]>(
    `/notification/user/${user_id}/unread`
  );
  return res.data ?? [];
}
...
export async function getUnreadCount(
  ...
  const res = await api.get<{ total: number }>(
    `/notification/user/${user_id}/unread/count`
  );
 ...
 * ======================================================= */
export async function bulkMarkNotificationRead(
